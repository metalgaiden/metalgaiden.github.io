...
Let’s take a look again at the three strategies in the context of what they can teach the player. If the strategy is analogous to some real world music analysis method then we can say that it will mirror the “experience” of doing that form of analysis. 

The first strategy, close listening, is quite obviously analogous. Even if the algorithm they discover in game is different to one in real life it can still walk the player through the process of forming a particular taste.

The second strategy, looking at the shapes on the cards, was based on the idea of “Tonic” “Predominant” and “Dominant” chord functions. Again, this is never made explicit, so the gameplay of noticing the shapes and assuming that they can relate to each other is not the same as, but analogous to, real world analysis. In the real world this is a heuristic that works for simple progressions, but as soon as you start mixing keys and using less common chords you will run into exceptions. In my experience, players would often be too quick to remove this strategy once they saw a contradiction, even if it was useful for the first couple hands they played. If players could learn to use a less than perfectly accurate strategy to inform future action this would be quite useful, but some convincing may be necessary.

The last strategy, using the numbers and notations to fully understand what’s happening, is more likely to be a number of strategies hiding as a single one. For example, I could imagine giving the player access to the notes in each chord, and tell them how many notes changed between the two chords. They can already do this if they take full notes and write out each chord, but it’s not immediately visible to an average player. I think there is potential here, but it needs to be teased out with more mechanics and tools that suggest smaller aspects of the overall picture.
